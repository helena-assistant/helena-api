...
const handler = async (event) => {
    ...
    if (isNaN(rate)) {
        return responseHandler(400, { response: 'rate is not a number' });
    }

    if (rate < 1 || rate > 5) {
        return responseHandler(400, {
            response: 'rate is invalid, please enter a value between 1 and 5'
        });
    }

    await deps.updateRating({ rate })

    return responseHandler(201, { response: 'OK' });
};
